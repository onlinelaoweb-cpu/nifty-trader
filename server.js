...
// 13 Sep refactor (Phase 1) — pure helpers extracted verbatim, zero logic
...
// 13 Sep — security headers. contentSecurityPolicy is disabled because
// public/index.html is a single-file frontend built entirely on inline
// <script>/<style> blocks — the default CSP would block the whole UI.
// Everything else (X-Frame-Options, noSniff, HSTS, etc.) stays on.
app.use(helmet({ contentSecurityPolicy: false }));
// 13 Sep — rate limiting on /api. Fastest legitimate poller in the frontend
// hits every 3s (~20/min); this allows generous headroom for multiple tabs
// while still blocking brute-force APP_TOKEN guessing / flooding.
// /api/stream is excluded — it's a single long-lived SSE connection per
// tab, not a repeated request, and browsers can auto-reconnect it on
// network blips, which would otherwise trip the limiter unfairly.
...
// isMarketOpen(), getIST(), isSafeEntryWindow() — moved to
// src/utils/timeWindows.js (13 Sep refactor, Phase 1), imported above.
...
// isNSEMarketDay() — moved to src/utils/timeWindows.js (13 Sep refactor, Phase 1).
...
// isCrudeSessionOpen() and getActiveSession() — moved to
// src/utils/timeWindows.js (13 Sep refactor, Phase 2), imported above.
...
// computeCrudeIndicators() — moved to src/utils/pureCalc.js (13 Sep refactor,
// Phase 1), imported above.
...
// CRUDE_MIN_CONFIDENCE / CRUDE_HIGH_CONVICTION_MIN — moved to
// src/utils/pureCalc.js alongside computeCrudeSignal() (13 Sep refactor, Phase 2).
...
// aggregateCandles() and crudeTFDirection() — moved to src/utils/pureCalc.js
// (13 Sep refactor, Phase 1), imported above.

// computeCrudeMTF() and computeCrudeSignal() — moved to src/utils/pureCalc.js
// (13 Sep refactor, Phase 2), imported above.
...
// pcrLabel() and pcrScore() — moved to src/utils/pureCalc.js (13 Sep
// refactor, Phase 1), imported above.
...
// computeMurarkaZone() — moved to src/utils/pureCalc.js (13 Sep refactor,
// Phase 2), imported above.
...
// calculateADX() — moved to src/utils/pureCalc.js (13 Sep refactor, Phase 1),
// imported above. Its _lastWarn throttle property lives on the same function
// object regardless of which file defines it, so that behavior is unchanged.
...
// detectLiquiditySweepReversal() — moved to src/utils/pureCalc.js (13 Sep
// refactor, Phase 2), imported above.
...
// daysToNextExpiry() — moved to src/utils/timeWindows.js (13 Sep refactor,
// Phase 2), imported above.
...
// buildTradeCoach() and buildScalpPlan() — moved to src/utils/pureCalc.js
// (13 Sep refactor, Phase 2), imported above.
...
// buildEngineChecklist() — moved to src/utils/pureCalc.js (13 Sep refactor,
// Phase 2), imported above.
...
// computeDecayedConfidence() and bsEstimate() — moved to
// src/utils/pureCalc.js (13 Sep refactor, Phase 2), imported above.
...
// withTimeout() — moved to src/utils/pureCalc.js (13 Sep refactor, Phase 2),
// imported above.
