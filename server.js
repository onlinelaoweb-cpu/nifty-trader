...
function combineSignals(indicators) {
    ...
    if (marketState.atmPcr !== null) {
        if      (marketState.atmPcrSignal === 'BULLISH') { bull += 2; reasons.push(`ATM PCR ${marketState.atmPcr} — Bullish ✅`); }
        else if (marketState.atmPcrSignal === 'BEARISH') { bear += 2; reasons.push(`ATM PCR ${marketState.atmPcr} — Bearish ⚠️`); }
    }
    ...
    if (marketState.vix > 30) { signal = 'WAIT'; confidence = 0; reasons.push('VIX > 30 — Avoid option buying!'); }
...
async function checkTelegramAlerts(newSignal) {
    ...
    if (h === 15 && m >= 30 && !closeSummarySent) {
        ...
        setTimeout(() => { morningSummarySent = false; closeSummarySent = false; vixAlertSent = false; }, 6 * 60 * 60 * 1000);
        return;
    }
...
async function updatePrice(price, change, changePct, source) {
    ...
    if (source === 'yahoo') {
        console.log(`NIFTY:${price} RSI:${indicators.rsi||'--'} EMA9:${indicators.ema9||'--'} VWAP:${indicators.vwap||'--'} → ${signal}(${confidence}%)`);
    }
...
async function refreshMarketData() {
    ...
    if (niftyData?.price > 0) {
        if (marketState.source !== 'websocket') {
            await updatePrice(niftyData.price, niftyData.change, niftyData.changePct, 'yahoo');
        } else {
...
async function refreshMTF() {
    try {
        ...
        if (mtfData) {
            marketState.mtf = {
                signal: mtfData.mtfSignal, strength: mtfData.mtfStrength,
                confidence: mtfData.mtfConfidence, aligned: mtfData.aligned,
                bullCount: mtfData.bullCount, bearCount: mtfData.bearCount,
                tf5m: mtfData.tf5m, tf15m: mtfData.tf15m, tf1h: mtfData.tf1h
            };
            console.log(`MTF: ${mtfData.mtfSignal} (${mtfData.mtfStrength})`);
        }
    } catch (err) { console.error('MTF error:', err.message); }
}
...
app.get('/api/health',  (req, res) => res.json({
    ...
    nifty: marketState.nifty, rsi: marketState.rsi,
    mtfSignal: marketState.mtf.signal, vix: marketState.vix,
...
app.post('/api/pcr', (req, res) => {
    ...
    if (pcr     != null) { marketState.pcr       = parseFloat(pcr);    marketState.pcrSignal    = pcrLabel(marketState.pcr);    console.log(`PCR: ${marketState.pcr} → ${marketState.pcrSignal}`); }
    if (atmPcr  != null) { marketState.atmPcr    = parseFloat(atmPcr); marketState.atmPcrSignal = pcrLabel(marketState.atmPcr); console.log(`ATM PCR: ${marketState.atmPcr} → ${marketState.atmPcrSignal}`); }
    res.json({ success: true });
});

app.post('/api/telegram/test', async (req, res) => {
    if (!isConfigured()) return res.json({ success: false, msg: 'Telegram not configured' });
    await sendMorningSummary(marketState);
...
async function initializeLiveData() {
    ...
    else      { console.log('Yahoo Finance fallback active'); setTimeout(initializeLiveData, 30000); }
}
