'use strict';
// levels.js — Support/Resistance pivot levels
//
// UPGRADE v2 (June 2026):
//  • Now uses 1-year historical data (nifty_daily_history) for:
//    - 52-week High/Low (most important institutional levels)
//    - Monthly High/Low (3-month range)
//    - Swing High/Low (last 30 days, 5-candle pivot)
//  • These add 4-6 extra powerful S/R levels that were missing before
//  • Daily pivot math unchanged — still uses prev-day OHLC
//
// Candle source: in-memory history from Angel WebSocket (getCandleHistory).
// Falls back to NSE allIndices for prev-day OHLC if memory has < 2 days.

const { fetchAllIndices } = require('./yahooFetch');
const { getCandleHistory } = require('./indicators');

// ── Historical data module (optional — graceful degradation if not available) ─
let _histModule = null;
try {
    _histModule = require('./historicalData');
} catch (_) {
    // historicalData.js not present — levels work without it (just fewer levels)
}

// Cache weekly OHLC — refresh once per day
let _weeklyCache   = null;
let _weeklyCacheAt = 0;

async function getWeeklyOHLC() {
    if (_weeklyCache && Date.now() - _weeklyCacheAt < 60 * 60 * 1000) {
        return _weeklyCache;
    }
    try {
        const { nseNiftyDaily } = require('./yahooFetch');
        const days = await nseNiftyDaily(7);
        if (days && days.length >= 3) {
            _weeklyCache = {
                high : Math.max(...days.map(d => d.high)),
                low  : Math.min(...days.map(d => d.low)),
                open : days[0].open,
                close: days[days.length - 1].close,
            };
            _weeklyCacheAt = Date.now();
            console.log(`[SR] Weekly OHLC: H=${_weeklyCache.high} L=${_weeklyCache.low} (${days.length} days)`);
            return _weeklyCache;
        }
    } catch (e) {
        console.warn('[SR] Weekly OHLC fetch failed:', e.message);
    }
    return null;
}

async function getPrevDayOHLC() {
    try {
        const indices = await fetchAllIndices();
        const row = indices.find(r => r.index === 'NIFTY 50' || r.indexSymbol === 'NIFTY 50');
        if (!row) return null;
        return {
            high : parseFloat(row.high  || row.yearHigh  || row.last),
            low  : parseFloat(row.low   || row.yearLow   || row.last),
            close: parseFloat(row.previousClose || row.last),
            open : parseFloat(row.open  || row.last),
        };
    } catch (e) { return null; }
}

function getPrevDayFromMemory(candles) {
    if (!candles || candles.length < 30) return null;
    const IST_OFFSET_MS = 5.5 * 60 * 60 * 1000;
    const nowIST   = new Date(Date.now() + IST_OFFSET_MS);
    const todayStr = nowIST.toISOString().slice(0, 10);
    const yesterday = candles.filter(c => {
        if (!c.ts) return false;
        const d = new Date(c.ts + IST_OFFSET_MS).toISOString().slice(0, 10);
        return d < todayStr;
    });
    const src = yesterday.length >= 10 ? yesterday : candles.slice(0, Math.floor(candles.length / 2));
    if (src.length === 0) return null;
    return {
        high : Math.max(...src.map(c => c.high)),
        low  : Math.min(...src.map(c => c.low)),
        close: src[src.length - 1].close,
        open : src[0].close,
    };
}

// ── Fetch 1-year historical levels (graceful: returns {} if histModule absent) ─
async function getHistoricalLevels(currentPrice) {
    if (!_histModule) return {};
    try {
        const [yearHL, monthly, swings] = await Promise.all([
            _histModule.get52WeekHighLow(),
            _histModule.getMonthlyLevels(),
            _histModule.getSwingLevels(),
        ]);
        return { yearHL, monthly, swings };
    } catch (e) {
        console.warn('[SR] Historical levels error:', e.message);
        return {};
    }
}

async function calculateSRLevels(currentPrice, maxPainData = null) {
    try {
        // ── Prev-day OHLC (for daily pivots) ─────────────────────────────────
        const memCandles = getCandleHistory();
        let ohlc = getPrevDayFromMemory(memCandles);
        if (!ohlc) ohlc = await getPrevDayOHLC();
        if (!ohlc) return null;

        const pdH = parseFloat(ohlc.high.toFixed(0));
        const pdL = parseFloat(ohlc.low.toFixed(0));
        const pdC = parseFloat(ohlc.close.toFixed(0));

        // Classic pivot points
        const pp = parseFloat(((pdH + pdL + pdC) / 3).toFixed(0));
        const r1 = parseFloat((2 * pp - pdL).toFixed(0));
        const r2 = parseFloat((pp + pdH - pdL).toFixed(0));
        const r3 = parseFloat((pdH + 2 * (pp - pdL)).toFixed(0));
        const s1 = parseFloat((2 * pp - pdH).toFixed(0));
        const s2 = parseFloat((pp - pdH + pdL).toFixed(0));
        const s3 = parseFloat((pdL - 2 * (pdH - pp)).toFixed(0));

        // ── Weekly OHLC ───────────────────────────────────────────────────────
        const weeklyOHLC = await getWeeklyOHLC();
        const wHigh = weeklyOHLC
            ? parseFloat(weeklyOHLC.high.toFixed(0))
            : parseFloat(Math.max(...(memCandles.length > 0 ? memCandles.map(c => c.high) : [pdH])).toFixed(0));
        const wLow = weeklyOHLC
            ? parseFloat(weeklyOHLC.low.toFixed(0))
            : parseFloat(Math.min(...(memCandles.length > 0 ? memCandles.map(c => c.low) : [pdL])).toFixed(0));

        // ── 1-Year historical levels ──────────────────────────────────────────
        const hist = await getHistoricalLevels(currentPrice);

        // ── Build levels array ────────────────────────────────────────────────
        const levels = [
            { price: r3,    type: 'R3',  label: 'Pivot R3',      strength: 1 },
            { price: r2,    type: 'R2',  label: 'Pivot R2',      strength: 2 },
            { price: wHigh, type: 'WH',  label: 'Week High',     strength: 3 },
            { price: pdH,   type: 'PDH', label: 'Prev Day High', strength: 3 },
            { price: r1,    type: 'R1',  label: 'Pivot R1',      strength: 2 },
            { price: pp,    type: 'PP',  label: 'Pivot Point',   strength: 3 },
            { price: s1,    type: 'S1',  label: 'Pivot S1',      strength: 2 },
            { price: pdL,   type: 'PDL', label: 'Prev Day Low',  strength: 3 },
            { price: wLow,  type: 'WL',  label: 'Week Low',      strength: 3 },
            { price: s2,    type: 'S2',  label: 'Pivot S2',      strength: 2 },
            { price: s3,    type: 'S3',  label: 'Pivot S3',      strength: 1 },
        ];

        // ── ADD: 52-Week High/Low (strongest institutional levels) ────────────
        if (hist.yearHL) {
            levels.push({ price: parseFloat(hist.yearHL.high.toFixed(0)), type: 'YH', label: `52W High (${hist.yearHL.highDate})`, strength: 5 });
            levels.push({ price: parseFloat(hist.yearHL.low.toFixed(0)),  type: 'YL', label: `52W Low (${hist.yearHL.lowDate})`,  strength: 5 });
        }

        // ── ADD: Monthly High/Low (last 3 months) ────────────────────────────
        if (hist.monthly && hist.monthly.length > 0) {
            for (const m of hist.monthly.slice(0, 2)) {  // last 2 months only
                levels.push({ price: parseFloat(m.high.toFixed(0)), type: 'MH', label: `Monthly High ${m.month}`, strength: 4 });
                levels.push({ price: parseFloat(m.low.toFixed(0)),  type: 'ML', label: `Monthly Low ${m.month}`,  strength: 4 });
            }
        }

        // ── ADD: Swing Highs/Lows (last 30 days, 5-candle pivot) ─────────────
        if (hist.swings) {
            for (const sh of (hist.swings.highs || [])) {
                levels.push({ price: sh.price, type: 'SH', label: `Swing High ${sh.date}`, strength: 2 });
            }
            for (const sl of (hist.swings.lows || [])) {
                levels.push({ price: sl.price, type: 'SL', label: `Swing Low ${sl.date}`, strength: 2 });
            }
        }

        // ── Max Pain ──────────────────────────────────────────────────────────
        if (maxPainData?.strike) {
            const onExpiry = maxPainData.expiryDay;
            levels.push({
                price   : maxPainData.strike,
                type    : 'MP',
                label   : onExpiry ? 'Max Pain 🎯 EXPIRY' : 'Max Pain',
                strength: onExpiry ? 5 : 4
            });
        }

        // ── Dedup + sort by price ─────────────────────────────────────────────
        // Keep highest-strength level when duplicates within 50 points
        const dedupedLevels = [];
        const sorted = levels
            .filter(l => l.price > 0 && !isNaN(l.price))
            .sort((a, b) => b.price - a.price);

        for (const lvl of sorted) {
            const nearby = dedupedLevels.find(x => Math.abs(x.price - lvl.price) < 50);
            if (!nearby) {
                dedupedLevels.push(lvl);
            } else if (lvl.strength > nearby.strength) {
                // Replace with stronger level
                const idx = dedupedLevels.indexOf(nearby);
                dedupedLevels[idx] = lvl;
            }
        }

        dedupedLevels.sort((a, b) => b.price - a.price);

        const above   = dedupedLevels.filter(l => l.price > currentPrice);
        const below   = dedupedLevels.filter(l => l.price < currentPrice);
        const nearRes = above.length > 0 ? above[above.length - 1] : null;
        const nearSup = below.length > 0 ? below[0] : null;

        const histSource = hist.yearHL ? '1Y+Pivots' : 'Pivots only';
        console.log(`S/R (${histSource}): PP=${pp} R1=${r1} S1=${s1} 52W-H=${hist.yearHL?.high?.toFixed(0)||'N/A'} | Res:${nearRes?.price} Sup:${nearSup?.price}`);

        return {
            pp, r1, r2, r3, s1, s2, s3,
            pdH, pdL, pdC,
            wHigh, wLow,
            yearHigh: hist.yearHL?.high  || null,
            yearLow : hist.yearHL?.low   || null,
            yearHighDate: hist.yearHL?.highDate || null,
            yearLowDate : hist.yearHL?.lowDate  || null,
            levels      : dedupedLevels,
            nearRes, nearSup,
            maxPain     : maxPainData || null,
            historicalData: !!hist.yearHL,  // flag: true when 1yr data available
            updatedAt   : new Date().toISOString(),
        };
    } catch (err) {
        console.error('S/R levels error:', err.message);
        return null;
    }
}

module.exports = { calculateSRLevels };